...
const navLinkClass = ({ isActive }) =>
  [
    ...
    isActive ? "bg-emerald-600 text-white" : "text-slate-200 hover:bg-white/10 hover:text-white",
  ].join(" ");
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="w-64 bg-slate-900 text-white hidden md:flex flex-col">
        <div className="px-5 py-5 border-b border-white/10">
          <Link to="/lecturer/dashboard" className="font-extrabold tracking-tight text-lg">
            Lecturer Dashboard
          </Link>
          <p className="text-xs text-slate-300 mt-1">{user?.name || "Lecturer"}</p>
        </div>
